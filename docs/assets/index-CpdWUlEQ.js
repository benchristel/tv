...
`))+`
`+suffix}__name(formatStructure,"formatStructure");async function runTests(tests){const results=[];for(const test of tests){const error=await errorFrom(test.fn),instrumentLog=debugLogs.map(args=>({type:"debug",args}));debugLogs.length=0,results.push({test,error,instrumentLog})}return{results}}__name(runTests,"runTests");function errorFrom(f){let caught;try{const result=f();if(result instanceof Promise)return new Promise(resolve=>{result.then(()=>resolve()).catch(resolve)})}catch(e){caught=e}return Promise.resolve(caught)}__name(errorFrom,"errorFrom");const debugLogs=[];curry(__name(function(args,error){return error instanceof ExpectationFailure&&equals(args,error.expectArgs)},"isExpectationFailure"));const blankLine=`
