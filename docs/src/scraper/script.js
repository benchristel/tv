...
function scrapeVideos() {
  if (/\/watch\?|\/embed\//.test(window.location)) {
    return [
      ...
      document.querySelector("a.ytp-title-link").innerText.split("\n")[0].trim()
    ].join(" ");
  } else if (/\/playlist\?/.test(window.location)) {
    return [...document.querySelectorAll("ytd-playlist-video-renderer")].map((v) => {
    ...
    return [...document.querySelectorAll("ytd-rich-grid-media")].map((v) => {
