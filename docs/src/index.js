...
# bar`),V,[])},"strips spaces before comments"(){U(ke(`  #foo
  # bar`),V,[])},"parses actual data"(){U(ke(`
    ...
    `),V,[{videoId:"leb645Xu6uo",durationSeconds:654,title:"Captain Murderer - Emlyn Williams"},{videoId:"Ga8tNxnHjt4",durationSeconds:181,title:"Sut Wnaethoch Chi Sillafu Caernarfon? Y Dydd 1971"}])},"removes shorts"(){U(ke(`
      ...
      leb645Xu6uo 10:54 Captain Murderer - Emlyn Williams
    `),V,[{videoId:"leb645Xu6uo",durationSeconds:654,title:"Captain Murderer - Emlyn Williams"}])}});function Rg(e){return e.startsWith("#")}function Ce(e){let n=[1,60,3600,86400],a=e.split(":").map(t=>parseInt(t,10)).reverse();return bg(a,n).map(([t,i])=>t*i).reduce(Fg,0)}Re("parseDuration",{"0:00"(){U(Ce("0:00"),x,0)},"00:00"(){U(Ce("00:00"),x,0)},"1 second"(){U(Ce("0:01"),x,1)},"10 seconds"(){U(Ce("0:10"),x,10)},"1 minute"(){U(Ce("1:00"),x,60)},"1 minute 10 seconds"(){U(Ce("1:10"),x,70)},"10 minutes 10 seconds"(){U(Ce("10:10"),x,610)},"1 hour"(){U(Ce("1:00:00"),x,3600)},"1 hour 1 second"(){U(Ce("1:00:01"),x,3601)},"10 hours 1 second"(){U(Ce("10:00:01"),x,36001)},"1 day 1 second"(){U(Ce("1:0:00:01"),x,86401)}});function Lg(e){return e.trim()}function Hg(e){return e.length===0}var ut=(e,n=Infinity)=>a=>{if(typeof e=="string"&&e.length===0)throw"split(): empty delimiter not supported";let t=0,i=0,r=[],o=a;for(;r.length<n-1&&o.length>0;){let s=o.match(e);if(s==null)break;r.push(o.slice(0,s.index)),o=o.slice(s.index+s[0].length)}return r.push(o),r};Re("split",{"empty string"(){U(ut(" ")(""),V,[""])},"no limit"(){U(ut(" ")("a b c"),V,["a","b","c"])},limit(){U(ut(" ",2)("a b c"),V,["a","b c"])},regex(){U(ut(/ +/)("a b   c"),V,["a","b","c"])}});function bg(e,n){let a=[];for(let t=0;t<e.length&&t<n.length;t++)a.push([e[t],n[t]]);return a}function Fg(e,n){return e+n}var S1=ke(`
...
`);var Oo=(...e)=>e.reduce(Yg),Fu=xg(Oo(ke,e=>({videos:e})))(["BecZdQvjGrY 54:49 In Gowan Ring - Compendium (Full Album)","B3oAx9VmAdA 43:52 In Gowan Ring - Hazel Steps Through A Weathered Home",`
  ...
  `,"i0djHJBAP3U 1:18:43 Huun-Huur-Tu - Live Berkely"]);Re("pipe",{"given one function"(){U(Oo(n=>n+1)(1),x,2)},"given two functions"(){let e=n=>n+1;U(Oo(e,e)(1),x,3)}});function Yg(e,n){return(...a)=>n(e(...a))}function xg(e){return n=>n.map(e)}function xu(e){let[n,a]=we(Yu);return F(Un,null,e.children(n),F("button",{onClick:()=>a(Yu)},"Channel 1"),F("button",{onClick:()=>a(Qg)},"Channel 2"),F("button",{onClick:()=>a(jg)},"Channel 3"))}var Yu=Ti(Hu.map(e=>({videos:[e]}))),Qg=Ti(bu.map(e=>({videos:[e]}))),jg=Ti(Fu);function Qu(){return F("div",{className:"App"},F("div",{className:"bezel"},F(xu,null,e=>F("div",{className:"screen"},F(zg,{channel:e},n=>F(ku,{id:"player-container"},a=>F(Mu,{broadcast:n,player:a})))))))}function zg(e){let{channel:n,children:a}=e,[t,i]=we(+new Date),[r,o]=we(!1);return Tu(()=>i(+new Date),1e3),F(Un,null,!r&&F("button",{id:"start",onClick:()=>o(!0)},"Play"),a(r?n.getBroadcast(t):{type:"nothing"}))}function ju(){let[e,n]=we("");return je(()=>{Ku(Du()).then(Bu).then(n).catch(a=>n(a.message))},[]),/fail/i.test(e)?F("div",{className:"TestResults"},F("code",null,F("pre",null,e))):null}Ao(F(Qu,null),document.getElementById("app"));Ao(F(ju,null),document.getElementById("test-results"));
/*
object-assign
(c) Sindre Sorhus
@license MIT
