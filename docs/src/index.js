...
`);function Pu(){return ue("div",{class:"App"},ue(Iu,{id:"player-container"},e=>ue(Cg,{player:e,model:vg})))}var Ig=wu.map(e=>({videos:[e]})),vg=Eg();function Eg(){let e={};return{getTargets:a};function a(s){let l=Math.floor(s/1e3),d=(l-8*3600)%(24*3600),f=l-d,v=n(String(f)),p=0,y;for(y of v)if(p+=y.durationSeconds,p>=d)return{targetVideoId:y.videoId,targetTime:y.durationSeconds-(p-d)};return{targetTime:10,targetVideoId:"ubFq-wV3Eic"}}function n(s){return e[s]||(e={},e[s]=t(s)),e[s]}function t(s){let l=r(o(s)),d=0,f=[];for(;d<24*3600;){let v=i(Ig,l());d+=v.videos.map(p=>p.durationSeconds).reduce(wg,0),f.push(...v.videos)}return f}function i(s,l){return s[Math.floor(l*s.length)]}function r(s){return function(){var l=s+=1831565813;return l=Math.imul(l^l>>>15,l|1),l^=l+Math.imul(l^l>>>7,l|61),((l^l>>>14)>>>0)/4294967296}}function o(s){let l=1779033703,d=3144134277,f=1013904242,v=2773480762;for(let p=0,y;p<s.length;p++)y=s.charCodeAt(p),l=d^Math.imul(l^y,597399067),d=f^Math.imul(d^y,2869860233),f=v^Math.imul(f^y,951274213),v=l^Math.imul(v^y,2716044179);return l=Math.imul(f^l>>>18,597399067),d=Math.imul(v^d>>>22,2869860233),f=Math.imul(l^f>>>17,951274213),v=Math.imul(d^v>>>19,2716044179),(l^d^f^v)>>>0}}function Cg(e){Pg(r,1e3);let[a,n]=Ba(!1),{player:t,model:i}=e;function r(){if(!a)return;let o=t.getPlayerState(),s=Wg(t.getVideoUrl());switch(o){case Fe.BUFFERING:return;case Fe.CUED:case Fe.UNSTARTED:case Fe.ENDED:case Fe.PLAYING:case Fe.PAUSED:}let l=+new Date,{targetVideoId:d,targetTime:f}=i.getTargets(l);if(s!==d){console.debug("video ID changed",s,d,f),f<5?(console.debug("target time is near 0; starting from beginning"),t.cueVideoById(d,0)):t.cueVideoById(d,f);return}if(!(o===Fe.PLAYING||o===Fe.ENDED)&&s){console.debug("video stopped; playing it"),t.playVideo();return}let v=t.getCurrentTime();Bg(v,f)>=5&&(console.debug("time is off; seeking",v,f),t.seekTo(f))}return a?null:ue("button",{id:"start",onClick:()=>{t.cueVideoById("ubFq-wV3Eic",10),t.playVideo(),n(!0)}},"Play")}function Pg(e,a){let n=ln(),t=ln();t.current=e,Wa(()=>(n.current=setInterval(()=>{var i;return(i=t.current)==null?void 0:i.call(t)},a),()=>clearInterval(n.current)),[a])}function Wg(e){var a;return(a=/v=(.{11})/.exec(e))==null?void 0:a[1]}function Bg(e,a){return Math.abs(e-a)}function wg(e,a){return e+a}function Wu(){let[e,a]=Ba("");return Wa(()=>{Au(Cu()).then(yu).then(a).catch(n=>a(n.message))},[]),/fail/i.test(e)?ue("div",{class:"TestResults"},ue("code",null,ue("pre",null,e))):null}So(ue(Pu,null),document.getElementById("app"));So(ue(Wu,null),document.getElementById("test-results"));
/*
object-assign
(c) Sindre Sorhus
@license MIT
